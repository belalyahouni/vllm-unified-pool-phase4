...
class FusedMoE(CustomOp):
    ...
    def unified_pool_stage1(self) -> dict[str, Any]:
        ...
        w13_bytes_per_expert = cpu_w13.element_size() * cpu_w13[0].numel()
        w2_bytes_per_expert = cpu_w2.element_size() * cpu_w2[0].numel()
        return {
    ...
    def _forward_with_unified_pool(
        ...
        Same shape as _forward_with_expert_cache, just with super-block
        ids instead of cache slot indices and a strided pool view in
        place of a packed cache tensor. The view's stride(0) equals
        expert_slot_bytes / element_size (== F pages), so the unmodified
        kernel walks one super-block per expert row even though experts
        aren't packed contiguously across the buffer.
        """
        ...
        try:
            ...
            # (non-sentinel) entry in super_block_id_at. int64 indexing
            # matches the kernel's int64 cast.
            remapped_ids = pool.super_block_id_at[topk_ids]

            # Optional one-shot sanity check: at the very first forward of
            # layer 0, compare each remapped super-block's pool view against
            # the CPU truth for the expert it should hold.
            if (
                os.environ.get("VLLM_UNIFIED_POOL_PARANOID", "0") == "1"
                and pool.layer_idx == 0
                and pool.forward_count == 0
            ):
                unique_sbs = remapped_ids.unique().tolist()
                for sb in unique_sbs:
                    eid = pool.expert_at_super_block.get(sb)
                    if eid is None:
                        raise RuntimeError(
                            f"L0: kernel will read super-block {sb} but no "
                            f"expert is mapped there"
                        )
                    w13_truth = pool.cpu_w13[eid].to(pool.device)
                    w13_actual = pool.pool_w13_view[sb]
                    if not torch.equal(w13_actual, w13_truth):
                        raise RuntimeError(
                            f"L0: pool_w13_view[{sb}] does not match "
                            f"cpu_w13[E{eid}] at kernel-call time. Bytes "
                            f"corrupted between warm-up and forward."
                        )
                    w2_truth = pool.cpu_w2[eid].to(pool.device)
                    w2_actual = pool.pool_w2_view[sb]
                    if not torch.equal(w2_actual, w2_truth):
                        raise RuntimeError(
                            f"L0: pool_w2_view[{sb}] does not match "
                            f"cpu_w2[E{eid}] at kernel-call time."
                        )
                logger.info(
                    "Phase-4 paranoid: L0 forward 0 verified pool views vs CPU "
                    "truth for %d unique super-block ids before kernel call.",
                    len(unique_sbs),
                )
            ...
            try:
                ...
                self.global_num_experts = pool.num_super_blocks
