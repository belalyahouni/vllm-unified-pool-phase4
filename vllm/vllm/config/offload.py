...
class OffloadConfig:
    ...
    side to evict under pressure. The fused MoE kernel reads weights
    directly out of the pool through strided views; topk_ids is rewritten
    per layer to point at block ids. Requires expert_offload=True,
    enable_prefix_caching=True, tensor_parallel_size=1,
    pipeline_parallel_size=1."""

    expert_pool_page_tokens: int = Field(default=16, ge=16)
    """Phase 4: size (in tokens) of one unified-pool page / KV block when
    --expert-unified-pool is set. Must be a multiple of 16 (the fused MoE
    kernel's block granularity). An expert now spans a *super-block* of
    F = expert_slot_bytes // page_size_bytes contiguous pages, where
    page_size_bytes = expert_pool_page_tokens * bytes_per_token. Smaller
    pages reduce internal fragmentation of the KV side (the tail of a
    partially-filled block is no longer a whole expert's worth of memory)
    at the cost of a larger super-block F and a bigger block table. The
    ...
    def validate_offload_config(self) -> "OffloadConfig":
        ...
        if self.expert_unified_pool and not self.expert_offload:
            raise ValueError("--expert-unified-pool requires --expert-offload.")

        if self.expert_unified_pool and self.expert_pool_page_tokens % 16 != 0:
            raise ValueError(
                f"--expert-pool-page-tokens ({self.expert_pool_page_tokens}) "
                f"must be a multiple of 16 (the fused MoE kernel block size)."
            )
