...
class GPUModelRunner(
    ...
    def _assert_attention_backend_kv_layout(self) -> None:
        ...
        # would silently corrupt live KV. The kernel reads the pool
        # directly here, so the corruption is invisible at runtime.
    ...
    def _unified_pool_stage1(self) -> None:
        ...
        Runs before profiling and KV cache allocation. Asks each
        FusedMoE module to pin its experts to CPU, then derives a
        block_size_tokens such that one KV page equals one expert
        slot's bytes and writes it back to cache_config.block_size.
        """
        from vllm.model_executor.layers.fused_moe.layer import FusedMoE
        ...
        if not self.cache_config.enable_prefix_caching:
            raise ValueError("--expert-unified-pool requires --enable-prefix-caching.")
        ...
        if self.scheduler_config.max_num_batched_tokens != 1:
            raise ValueError(
                ...
                "mid-call." % self.scheduler_config.max_num_batched_tokens
            )
        if getattr(self.scheduler_config, "async_scheduling", False):
            # The kernel reads pool bytes directly, and the pinning
            # contract assumes scheduler KV allocation can't run
            # while a forward is in flight. With async_scheduling
            # the metadata invariants would not hold.
            raise ValueError(
        ...
        if not moe_modules:
            raise ValueError(
                "--expert-unified-pool was set, but the model has no FusedMoE modules."
            )
        ...
        if not kv_specs:
            raise ValueError("--expert-unified-pool: model has no KV cache layers.")
        ...
        if expert_slot_bytes % bytes_per_token != 0:
            raise ValueError(
                ...
                "a multiple of KV bytes-per-token (%d B). Cannot align "
                "pages without padding." % (expert_slot_bytes, bytes_per_token)
            )

        # Phase 4: the page (== one KV block) is a tunable number of tokens,
        # and an expert spans a *super-block* of F contiguous pages. Phase 3
        # is the special case where the page is the whole expert (F == 1),
        # obtained by setting --expert-pool-page-tokens to the expert's size
        # in tokens (expert_slot_bytes // bytes_per_token).
        page_tokens = self.vllm_config.offload_config.expert_pool_page_tokens
        kernel_block_size = 16
        if page_tokens % kernel_block_size != 0:
            raise ValueError(
                "--expert-pool-page-tokens=%d is not a multiple of the "
                "kernel block size %d." % (page_tokens, kernel_block_size)
            )
        page_size_bytes = page_tokens * bytes_per_token
        if expert_slot_bytes % page_size_bytes != 0:
            expert_tokens = expert_slot_bytes // bytes_per_token
            raise ValueError(
                "--expert-unified-pool: expert slot size (%d B = %d tokens) "
                "is not a multiple of the page size (%d B = %d tokens). "
                "Choose --expert-pool-page-tokens so that the expert's "
                "token count (%d) is a multiple of it."
                % (
                    expert_slot_bytes,
                    expert_tokens,
                    page_size_bytes,
        ...
        pages_per_super_block = expert_slot_bytes // page_size_bytes
        block_size_tokens = page_tokens
        ...
        logger.info(
            "UnifiedPool Stage 1 (Phase 4): %d MoE layers, expert_slot=%d B, "
            "bytes/token=%d, page=%d tokens (%d B), F=%d pages/super-block, "
            "block_size: %s -> %d tokens. No staging tensor — kernel reads "
            "the pool buffer directly through an F-strided super-block view.",
            len(moe_modules),
            expert_slot_bytes,
            bytes_per_token,
            page_tokens,
            page_size_bytes,
            pages_per_super_block,
            old_block_size,
            block_size_tokens,
        )
        ...
        self._unified_pool_page_size_bytes = page_size_bytes
        self._unified_pool_pages_per_super_block = pages_per_super_block
    ...
    def setup_unified_pool(self, block_pool) -> int:
        ...
        if not getattr(self, "_unified_pool_moe_modules", None):
            raise RuntimeError("setup_unified_pool called before Stage 1 ran.")
        if not self._unified_pool_kv_cache_raw_tensors:
            raise RuntimeError(
                "setup_unified_pool called before KV cache tensors were allocated."
            )
        ...
        for layer_idx, raw_kv in attn_layers.items():
            ...
            kv_pool_buffers[layer_idx] = raw_kv.narrow(0, 0, required_bytes)

        manager = UnifiedPoolManager(
            block_pool=block_pool,
            device=self.device,
            pages_per_super_block=self._unified_pool_pages_per_super_block,
        ...
        for moe_module, meta in zip(
            ...
            pool = UnifiedPool(
                ...
                page_size_bytes=page_size_bytes,
                expert_slot_bytes=self._unified_pool_expert_slot_bytes,
        ...
        pages_per_super_block = self._unified_pool_pages_per_super_block
        # An expert occupies a super-block of F contiguous pages. Super-block
        # 0 (pages [0, F)) contains BlockPool's null block (page 0) and can
        # never be fully assembled, so it is reserved. Warm-up shares one
        # super-block id across all layers per expert, so warming warm_count
        # experts costs warm_count * F pages in total.
        num_super_blocks = block_pool.num_gpu_blocks // pages_per_super_block
        num_super_blocks_available = num_super_blocks - 1  # reserve s=0
        if warm_count > num_super_blocks_available:
            raise RuntimeError(
                f"UnifiedPool: warm-up requires {warm_count} super-blocks "
                f"({warm_count * pages_per_super_block} pages) but only "
                f"{num_super_blocks_available} are available "
                f"(num_gpu_blocks={block_pool.num_gpu_blocks}, "
                f"F={pages_per_super_block}, num_super_blocks="
                f"{num_super_blocks}, minus reserved super-block 0). "
                f"Reduce --expert-cache-size, increase "
                f"--num-gpu-blocks-override, or raise "
                f"--expert-pool-page-tokens."
            )
