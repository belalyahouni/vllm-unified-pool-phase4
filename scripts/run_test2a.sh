...
# Phase 1: alternating prefixes, 10 prompts, --num-warmups 1
run_phase1() {
...
# Phase 2: random, 6 prompts, --num-warmups 0 (preserve pool state)
run_phase2() {
  local port=$1 outname=$2
  "$VLLM" bench serve --backend vllm --host 127.0.0.1 --port $port \
    --endpoint /v1/completions --model "$MODEL" \
...
# Run a single cell (boots server, phase1, phase2, shutdown).
# args: cell_name gpu port cache mode trace
#   mode: static|unified
run_cell() {
...
if [ "$ROUND" = "static" ] || [ "$ROUND" = "all" ]; then
  echo "=== 2A static cells ==="
  echo "  Phase 1 of round: prefix-tuned (cache=20, GPU0) + middle (cache=40, GPU1) ==="
  run_cell test2A_static_prefix_tuned 0 8000 20 static &
  run_cell test2A_static_middle 1 8001 40 static &
  wait
  echo "  Phase 2 of round: expert-tuned (cache=64, GPU0) ==="
  run_cell test2A_static_expert_tuned 0 8000 64 static
  echo "=== 2A static cells done ==="
fi

if [ "$ROUND" = "unified-middle" ] || [ "$ROUND" = "all" ]; then
  echo "=== 2A unified-from-middle (cache=40 init) — latency (GPU0) + trace (GPU1) ==="
  run_cell test2A_unified_from_middle 0 8000 40 unified 0 &
  run_cell test2A_unified_from_middle 1 8001 40 unified 1 &
  wait
  echo "=== 2A unified-from-middle done ==="
fi

if [ "$ROUND" = "unified-prefix" ] || [ "$ROUND" = "all" ]; then
  echo "=== 2A unified-from-prefix (cache=20 init, ablation) — latency (GPU0) + trace (GPU1) ==="
  run_cell test2A_unified_from_prefix 0 8000 20 unified 0 &
  run_cell test2A_unified_from_prefix 1 8001 20 unified 1 &
  wait
  echo "=== 2A unified-from-prefix done ==="
fi

if [ "$ROUND" = "unified-expert" ] || [ "$ROUND" = "all" ]; then
  echo "=== 2A unified-from-expert (cache=64 init, ablation) — latency (GPU0) + trace (GPU1) ==="
  run_cell test2A_unified_from_expert 0 8000 64 unified 0 &
  run_cell test2A_unified_from_expert 1 8001 64 unified 1 &
  wait
  echo "=== 2A unified-from-expert done ==="
fi

echo "=== Test 2A complete ==="
