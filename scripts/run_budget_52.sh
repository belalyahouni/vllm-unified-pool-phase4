#!/bin/bash
# M=52 to localise the cliff between M=48 (unified fails) and M=56 (unified wins).
set -u
SEED=1
...
# M=52 splits: (16,36), (24,28), (40,12); unified init=26
M=52
echo ""
echo "############################################"
echo "### Budget M=$M (static splits: (16,36)/(24,28)/(40,12); unified init=26)"
echo "############################################"
echo "=== Round 1: static cache=16 (GPU0) + static cache=24 (GPU1) ==="
run_cell budget${M}_static_C16 0 8000 static 16 36 &
run_cell budget${M}_static_C24 1 8001 static 24 28 &
wait
echo "=== Round 2: static cache=40 (GPU0) + unified pool=$M (GPU1) ==="
run_cell budget${M}_static_C40 0 8000 static 40 12 &
run_cell budget${M}_unified_init26 1 8001 unified 26 $M &
wait
