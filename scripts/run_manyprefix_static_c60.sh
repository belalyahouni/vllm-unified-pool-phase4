#!/bin/bash
# Run static C=60 (KV=4) cell of the many-prefixes test on GPU 1.
set -u
SEED=1
...
CUDA_VISIBLE_DEVICES=$GPU setsid "$VLLM" serve "$MODEL" \
  --port $PORT \
  --expert-offload --expert-cache-size 60 \
  ...
  --block-size 1536 --num-gpu-blocks-override 4 \
  --gpu-memory-utilization 0.6 \
  > "$LOGS/manyprefix_static_C60_seed${SEED}_g${GPU}.log" 2>&1 &
PID=$!
...
nvidia-smi --query-gpu=index,memory.used --format=csv,noheader -i $GPU \
  > "$LOGS/manyprefix_static_C60_seed${SEED}_idle_mem.txt"
...
run_phase "manyprefix_static_C60_phase1_seed${SEED}.json" \
  > "$LOGS/bench_manyprefix_static_C60_phase1_seed${SEED}_g${GPU}.log" 2>&1
echo "[static C=60 g${GPU}] phase1 (cold) exit: $?"
run_phase "manyprefix_static_C60_phase2_seed${SEED}.json" \
  > "$LOGS/bench_manyprefix_static_C60_phase2_seed${SEED}_g${GPU}.log" 2>&1
echo "[static C=60 g${GPU}] phase2 (warm) exit: $?"
