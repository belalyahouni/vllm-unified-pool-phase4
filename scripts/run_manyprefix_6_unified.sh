...
PROMPTS=$ROOT/prompts/many_prefixes.jsonl   # has 10 entries; we use --num-prompts 6
...
echo "=== 6-prefix unified test (pool=64 init=40) — latency (GPU0) + trace (GPU1) ==="
PID0=$(start_unified 0 8000 0 manyprefix6_unified_init40)
PID1=$(start_unified 1 8001 1 manyprefix6_unified_init40)
wait_for_server 8000 $PID0 || { shutdown $PID0; shutdown $PID1; exit 1; }
wait_for_server 8001 $PID1 || { shutdown $PID0; shutdown $PID1; exit 1; }
capture_idle_mem 0 manyprefix6_unified_init40

# Latency pass on GPU 0 (kept JSONs); trace pass on GPU 1 (discard JSONs, keep server log).
run_phase 8000 manyprefix6_unified_init40_phase1_seed${SEED}.json \
  > "$LOGS/bench_manyprefix6_unified_init40_phase1_seed${SEED}_g0.log" 2>&1 &
B0=$!
run_phase 8001 _discard_manyprefix6_unified_trace_phase1_seed${SEED}.json \
  > "$LOGS/bench_manyprefix6_unified_trace_phase1_seed${SEED}_g1.log" 2>&1 &
B1=$!
wait $B0; echo "[g0] phase1 (cold) exit: $?"
wait $B1; echo "[g1] phase1 trace exit: $?"

run_phase 8000 manyprefix6_unified_init40_phase2_seed${SEED}.json \
  > "$LOGS/bench_manyprefix6_unified_init40_phase2_seed${SEED}_g0.log" 2>&1 &
B0=$!
run_phase 8001 _discard_manyprefix6_unified_trace_phase2_seed${SEED}.json \
  > "$LOGS/bench_manyprefix6_unified_trace_phase2_seed${SEED}_g1.log" 2>&1 &
B1=$!
wait $B0; echo "[g0] phase2 (warm) exit: $?"
wait $B1; echo "[g1] phase2 trace exit: $?"

shutdown $PID0; shutdown $PID1
rm -f "$RESULTS/_discard_manyprefix6_unified_trace_phase1_seed${SEED}.json" \
      "$RESULTS/_discard_manyprefix6_unified_trace_phase2_seed${SEED}.json"
echo "=== 6-prefix unified test done ==="
